// TODO: Remove Hardcoded commands, Implement lua scripting.
// TODO: Implement cpplua support.
...
// TODO: Add support for >2 command operators???
struct cmd_t
{
    std::string command;
    std::string op1;
...
int readLine(int sock, char *buffer, int bufsize);
...
void *thread_proc(void *arg)
{
    ...
    while (!quit)
    {
        // get any commands the client has sent
        ...
        else if (status > 0)
        {
            ...
            {
                // TODO: Refactor to allow user-defined commands.
                if (cmd.command == "JOIN")
                {
                    if (joined)
                    {
    ...
    shutdown(sock, SHUT_RDWR); // Nice job outdated book;
    return arg;
}
...
// TODO: Re-implement this with a variable
cmd_t decodeCommand(const char *buffer)
{
    ...
    state = 0;
    for (int x = 0; x < std::strlen(buffer); x++)
    {
        if (buffer[x] == ' ' && state < 2)
        {
            state++;
        ...
        else
        {
            ...
            case 0:
                ...
                break;
            case 1:
                ret_cmd.op1 += buffer[x];
                break;
            default:
                ret_cmd.op2 += buffer[x];
            }
        }
    }
    return ret_cmd;
...
int pmsg_command(const cmd_t &cmd, const std::string &nickname, std::string &msg) 
{
    ...
    client_iter = client_list.find(cmd.op1);
    if (client_iter==client_list.end()) {
        msg = "202 UNKOWN NICKNAME";
    } else {
        (*client_iter).second.outbound.push_back("MSG "+nickname+" "+cmd.op2);
        msg = "100 OK";
    }
    pthread_mutex_unlock(&client_list_mutex);
}
    ...
    if (client_iter == client_list.end())
    {
        ...
        if (client_iter == client_list.end())
        {
                ...
                client_iter++)
            {
                ...
                // In any sane programming language, this would look like:
                // client.SendMessage("OP "+cmd.args[1]);
                msg = "100 OK";
            }
        }
    }
...
// g++ -o chatserver server.cpp -lpthread